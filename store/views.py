...
def store_home(request):
    """
    The Homepage:
    - If searching: Shows Product Results.
    - If browsing: Shows List of Categories.
    """
...
def product_detail(request, slug):
    ...
    related_products = Product.objects.filter(category=product.category, stock__gt=0).exclude(id=product.id)[:4]
    return render(request, 'store/detail.html', {'product': product, 'related_products': related_products})
